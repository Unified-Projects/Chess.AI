...
PieceWidget::PieceWidget(Type t, Colour c)
    : InformationWidget((1.f/3.f))
{
    // Nothing blocky setup due to not knowing constraints
    tex = GlobWin->Mapper->GetTex(t, c);
}

void PieceWidget::Update(){
    if(!GlobWin){ // Unlikely error but just in case
        return;
    }
}

void PieceWidget::Render(){
    bool noBind = false;
